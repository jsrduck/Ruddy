/*
 * Generate.cpp
 * https://github.com/kavon/llvm-statepoint-utils/blob/f810b5641b7a40a1c658be09b2f98e30b5d0e1f7/src/generate.c
...
frame_info_t* generate_frame_info(callsite_header_t* callsite, function_info_t* fn)
{
	...
	*/
	...
	auto frame = new frame_info_t(); //reinterpret_cast<frame_info_t*>(malloc(size_of_frame(numSlots)));
	...
	for (uint16_t i = 0; i < numSlots; i++, locations += 2)
	{
		...
		{
			if (frame->slots[k].offset == base->offset)
			{
				found = true;
				baseIdx = k;
				break;
...
std::unique_ptr<std::unordered_map<uint64_t, std::unique_ptr<frame_info_t>>> generate_table(BYTE* map)
{
	...
	auto table = std::make_unique<std::unordered_map<uint64_t, std::unique_ptr<frame_info_t>>>();
	...
	for (uint64_t _unused = 0; _unused < numCallsites; _unused++)
	{
		...
		auto info = generate_frame_info(callsite, currentFn);
		(*table)[info->retAddr] = std::unique_ptr<frame_info_t>(info);
...
{
	debugprint(L"Frame:\r\n");
	debugprint(L"\tRetAddr: %llx\r\n", frame->retAddr);
	debugprint(L"\tFrameSize: %ull\r\n", frame->frameSize);
	debugprint(L"\tNumSlots: %u\r\n", frame->numSlots);
	if (frame->slots.size() > 0)
	{
		for (auto& slot : frame->slots)
		{
			debugprint(L"\t\tSlot\tKind:%u\tOffset:%i\r\n", slot.kind, slot.offset);
		}
	}
}