//===- RewriteStatepointsForGC_Ruddy.cpp - Make GC relocations explicit ---------===//
//
...
//
// Adapted for use with Ruddy programming language
//
//===----------------------------------------------------------------------===//
//
...
#define GC_HEAP_ADDRESS_SPACE 0

using namespace llvm;

// Print the liveset found at the insert location
static cl::opt<bool> PrintLiveSet("spp-print-liveset-Ruddy", cl::Hidden,
	cl::init(false));
static cl::opt<bool> PrintLiveSetSize("spp-print-liveset-size-Ruddy", cl::Hidden,
	cl::init(false));

// Print out the base pointers for debugging
static cl::opt<bool> PrintBasePointers("spp-print-base-pointers-Ruddy", cl::Hidden,
	cl::init(false));

// Cost threshold measuring when it is profitable to rematerialize value instead
// of relocating it
static cl::opt<unsigned>
RematerializationThreshold("spp-rematerialization-threshold-Ruddy", cl::Hidden,
	cl::init(6));
...
static cl::opt<bool, true> ClobberNonLiveOverride("rs4gc-clobber-non-live-Ruddy",
	cl::location(ClobberNonLive),
	cl::Hidden);

static cl::opt<bool>
AllowStatepointWithNoDeoptInfo("rs4gc-allow-statepoint-with-no-deopt-info-Ruddy",
	cl::Hidden, cl::init(true));

/// The IR fed into RewriteStatepointsForGC_Ruddy may have had attributes and
/// metadata implying dereferenceability that are no longer valid/correct after
/// RewriteStatepointsForGC_Ruddy has run. This is because semantically, after
/// RewriteStatepointsForGC_Ruddy runs, all calls to gc.statepoint "free" the entire
...
PreservedAnalyses RewriteStatepointsForGC_Ruddy::run(Module &M,
...
class RewriteStatepointsForGC_RuddyLegacyPass : public ModulePass
{
	RewriteStatepointsForGC_Ruddy Impl;

public:
	static char ID; // Pass identification, replacement for typeid

	RewriteStatepointsForGC_RuddyLegacyPass() : ModulePass(ID), Impl()
	{
		initializeRewriteStatepointsForGC_RuddyLegacyPassPass(
...
char RewriteStatepointsForGC_RuddyLegacyPass::ID = 0;

ModulePass *llvm::createRewriteStatepointsForGC_RuddyLegacyPass()
{
	return new RewriteStatepointsForGC_RuddyLegacyPass();
}

INITIALIZE_PASS_BEGIN(RewriteStatepointsForGC_RuddyLegacyPass,
	"rewrite-statepoints-for-gc-Ruddy",
	"Make relocations explicit at statepoints", false, false)
	INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
	INITIALIZE_PASS_DEPENDENCY(TargetTransformInfoWrapperPass)
	INITIALIZE_PASS_END(RewriteStatepointsForGC_RuddyLegacyPass,
		"rewrite-statepoints-for-gc-Ruddy",
		"Make relocations explicit at statepoints", false, false)
...
static bool isGCPointerType(Type *T)
{
	if (auto *PT = dyn_cast<PointerType>(T))
		...
		return PT->getAddressSpace() == GC_HEAP_ADDRESS_SPACE;
	return false;
}
...
{
	SmallSet<T, 8> Seen;
	Vec.erase(remove_if(Vec, [&](const T &V)
	{
...
static void stripInvalidMetadataFromInstruction(Instruction &I)
{
	...
	// dropped.  This is because semantically, after RewriteStatepointsForGC_Ruddy runs,
...
static void stripNonValidDataFromBody(Function &F)
{
	...
	for (Instruction &I : instructions(F))
	{
		...
		// RewriteStatepointsForGC_Ruddy runs because there can be calls to gc.statepoint
...
bool RewriteStatepointsForGC_Ruddy::runOnFunction(Function &F, DominatorTree &DT,
	TargetTransformInfo &TTI,
	const TargetLibraryInfo &TLI)
{
